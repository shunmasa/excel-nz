...
const Background = () => {
    ...
    <svg id="home__svg" style={{ left: 0, top: "-10px" }} xmlns="http://www.w3.org/2000/svg" 
    // viewBox="0 0 1450 960"
    viewBox="0 0 1440 500">
 
      <defs>
        <motion.path className="stars_clip" id="a" d="" variants={clip_path_variants} animate={hasScrolled ? "closed" : "open"} transition={{
        ease: "easeInOut",
        duration: 0.5
    }}/>
...
const Stars = React.memo(() => {
    ...
    return STARS_COORDS.map((item, i) => {
        ...
        const y = useSpring(useTransform(scrollY, [20, 140], [0, 100 + Math.random() * 100]), { stiffness: 140, damping: 100 });
        return (<motion.path key={item} fill={stars_color[Math.round(Math.random() * (stars_color.length - 1))]} d={item} style={{ x, y, opacity }}/>);
    });
});
